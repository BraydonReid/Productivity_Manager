...
// List journals in date range
router.get('/', (req, res) => {
  const userId = req.user!.userId;
  const from = req.query.from as string | undefined;
  const to = req.query.to as string | undefined;
  const journals = listJournals(from, to, userId).map(formatJournal);
  res.json(journals);
});

// Get detailed session timeline for a date (raw data, no AI)
router.get('/:date/detail', (req, res) => {
  const userId = req.user!.userId;
  const data = getDayDetailedData(req.params.date, userId);
  res.json({
    sessions: data.sessions.map((s) => ({
      ...
      clipboardEntries: s.clipboardEntries.map((c) => ({
        content: c.content,
...
router.get('/:date', (req, res) => {
  const userId = req.user!.userId;
  const journal = getJournal(req.params.date, userId);
  if (!journal) {
    res.json(null);
    return;
  }
...
// Generate/regenerate journal for a date
