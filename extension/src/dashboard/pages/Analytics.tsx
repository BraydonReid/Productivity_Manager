...
export default function Analytics() {
  ...
  useEffect(() => {
    apiClient.get<AnalyticsData>('/sessions/analytics').then((res) => {
      if (res.success && res.data) {
        ...
        // Set empty defaults so page doesn't load forever
        setData({
          totalSessions: 0,
          totalTabs: 0,
          totalNotes: 0,
  ...
  async function loadMetrics() {
    ...
    const res = await apiClient.get<ProductivityMetric[]>(
      `/sessions/metrics?from=${from}&to=${to}`
    );
  ...
  // Calculate productivity insights from metrics
  const totalContextSwitches = metrics.reduce(
    (sum, m) => sum + m.contextSwitches, 0
  );
  const totalDeepWork = metrics.reduce(
    (sum, m) => sum + m.deepWorkMinutes, 0
  );
  const totalShallowWork = metrics.reduce(
    (sum, m) => sum + m.shallowWorkMinutes, 0
  );
  const deepWorkRatio =
  ...
  // Time recovered estimate: each restored session saves ~5 minutes
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold">Productivity Intelligence</h2>
      ...
      <div className="grid grid-cols-5 gap-3 mb-8">
        ...
        <StatCard label="Context Switches" value={totalContextSwitches} accent="yellow" />
        <StatCard label="Time Recovered" value={`${timeRecoveredMinutes}m`} accent="blue" />
      </div>

      <div className="grid grid-cols-2 gap-6 mb-8">
        {/* Deep vs Shallow work */}
        <section className="bg-gray-900 border border-gray-800 rounded-lg p-5">
          <h3 className="text-lg font-semibold mb-4">Work Quality</h3>
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-green-400">Deep Work</span>
                <span className="text-gray-400">{totalDeepWork}m</span>
              </div>
              <div className="w-full bg-gray-800 rounded-full h-3">
                <div className="bg-green-500 rounded-full h-3" style={{ width: `${deepWorkRatio}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-yellow-400">Shallow Work</span>
                <span className="text-gray-400">{totalShallowWork}m</span>
              </div>
              <div className="w-full bg-gray-800 rounded-full h-3">
                <div className="bg-yellow-500 rounded-full h-3" style={{ width: `${100 - deepWorkRatio}%` }} />
              </div>
            </div>
          </div>
          <p className="text-xs text-gray-500 mt-3">
            Deep work = 5+ minutes on a single domain without switching.
          </p>
        </section>
        ...
        <section className="bg-gray-900 border border-gray-800 rounded-lg p-5">
          ...
          <div className="flex gap-1 h-32 items-end">
            {Array.from({ length: 18 }, (_, i) => i + 6).map((h) => {
              ...
              const maxDeep = Math.max(
                ...Array.from(hourlyData.values()).map((v) => v.deep), 1
              );
                ...
                <div key={h} className="flex flex-col items-center justify-end flex-1 h-full">
                  <div
                    className={`w-full rounded-t ${
                      height > 60 ? 'bg-green-500' : height > 30 ? 'bg-blue-500' : 'bg-gray-700'
                    }`}
      ...
      {/* Daily breakdown */}
      {dailyData.size > 0 && (
        <section className="bg-gray-900 border border-gray-800 rounded-lg p-5 mb-8">
              ...
              .map(([date, d]) => {
                ...
                return (
                  <div key={date} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 flex h-4 bg-gray-800 rounded overflow-hidden">
                      <div className="bg-green-600" style={{ width: `${deepPct}%` }} />
                      <div className="bg-yellow-600" style={{ width: `${100 - deepPct}%` }} />
                    </div>
                    <span className="text-xs text-gray-500 w-16 text-right">{total}m total</span>
                    <span className="text-xs text-gray-600 w-16 text-right">{d.switches} switches</span>
                  </div>
                );
              })}
          </div>
          ...
          <div className="space-y-2">
            {data.topDomains.map(({ domain, visits, totalTime }) => (
              <div key={domain} className="flex items-center justify-between py-1">
                ...
                <span className="text-xs text-gray-500 w-20 text-right">{visits} visits</span>
                <span className="text-xs text-gray-500 w-20 text-right">{Math.round(totalTime / 60000)}m</span>
              </div>
            ))}
          </div>
        </section>
